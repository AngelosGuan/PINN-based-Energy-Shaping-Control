import configs.config_dof8 as config
import core.plot as plot
import core.sampling as sampling
import core.training as training
import core.utils as utils
import models.dof8.KMKhb_dof8 as models
import models.dof8.loss_dof8 as loss
import models.dof8.dynamics as dynamics
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Training for 8DOF dynamic model")
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_name", type=str, default="newMLP", help="Folder name to store the output within results folder."
    )
    ...
    X = sampling_with_contact_condition(num_samples=config.num_train_data, device=device, sampling_func=sampling.lhs_sampling)
    ...
    # verify on test sets
    # sobel
    test_set = sampling_with_contact_condition(num_samples=config.testset_size, device=device, sampling_func=sampling.sobol_sampling)
    ...
    # uniform
    test_set = sampling_with_contact_condition(num_samples=config.testset_size, device=device, sampling_func=sampling.uniform_sampling)
    ...
    # LHS
    test_set = sampling_with_contact_condition(num_samples=config.testset_size, device=device, sampling_func=sampling.lhs_sampling)
