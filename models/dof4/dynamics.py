...
def calculate_Mmtx(x, device, I1z, I2z, Ipz, Mp, Ms, Mt, l1, l2): 
    M =  torch.stack([
        torch.stack([2*I1x+2*I2x+Ipx+l1**2*(Mp+(3/2)*Ms+2*Mt)+(1/2)*l2**2*(2*Mp+4*Ms+3*Mt)+l1*l2*(2*(Mp+Ms)+3*Mt)*torch.cos(x[1])+(-1)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+(-1)*l1*(l2*(2*Ms+Mt)*torch.cos(x[1]+x[2])+l2*Ms*((-1)*torch.cos(x[3])+torch.cos(x[2]+x[3]))+l1*Ms*torch.cos(x[1]+x[2]+x[3])), 
        (1/4)*(4*I1x+8*I2x+4*Ipx+l1**2*Ms+l2**2*(4*Mp+8*Ms+6*Mt)+2*l1*l2*(2*(Mp+Ms)+3*Mt)*torch.cos(x[1])+(-4)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+(-2)*l1*(l2*(2*Ms+Mt)*torch.cos(x[1]+x[2])+2*l2*Ms*((-1)*torch.cos(x[3])+torch.cos(x[2]+x[3]))+l1*Ms*torch.cos(x[1]+x[2]+x[3]))), 
        (1/4)*(4*I1x+4*I2x+l1**2*Ms+l2**2*(4*Ms+Mt)+(-2)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+(-2)*l1*(l2*(2*Ms+Mt)*torch.cos(x[1]+x[2])+l2*Ms*((-2)*torch.cos(x[3])+torch.cos(x[2]+x[3]))+l1*Ms*torch.cos(x[1]+x[2]+x[3]))), 
        I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3])+(-2)*l2*torch.cos(x[2]+x[3])+(-2)*l1*torch.cos(x[1]+x[2]+x[3]))]), 

        torch.stack([(1/4)*(4*I1x+8*I2x+4*Ipx+l1**2*Ms+l2**2*(4*Mp+8*Ms+6*Mt)+2*l1*l2*(2*(Mp+Ms)+3*Mt)*torch.cos(x[1])+(-4)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+(-2)*l1*(l2*(2*Ms+Mt)*torch.cos(x[1]+x[2])+2*l2*Ms*((-1)*torch.cos(x[3])+torch.cos(x[2]+x[3]))+l1*Ms*torch.cos(x[1]+x[2]+x[3]))), 
            I1x+2*I2x+Ipx+(1/4)*(l1**2*Ms+l2**2*(4*Mp+8*Ms+6*Mt))+(-1)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+l1*l2*Ms*(torch.cos(x[3])+(-1)*torch.cos(x[2]+x[3])), 
            (1/4)*(4*I1x+4*I2x+l1**2*Ms+l2**2*(4*Ms+Mt)+(-2)*l2*(l2*(2*Ms+Mt)*torch.cos(x[2])+l1*Ms*((-2)*torch.cos(x[3])+torch.cos(x[2]+x[3])))), 
            I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3])+(-2)*l2*torch.cos(x[2]+x[3]))]), 

        torch.stack([(1/4)*(4*I1x+4*I2x+l1**2*Ms+l2**2*(4*Ms+Mt)+(-2)*l2**2*(2*Ms+Mt)*torch.cos(x[2])+(-2)*l1*(l2*(2*Ms+Mt)*torch.cos(x[1]+x[2])+l2*Ms*((-2)*torch.cos(x[3])+torch.cos(x[2]+x[3]))+l1*Ms*torch.cos(x[1]+x[2]+x[3]))), 
            (1/4)*(4*I1x+4*I2x+l1**2*Ms+l2**2*(4*Ms+Mt)+(-2)*l2*(l2*(2*Ms+Mt)*torch.cos(x[2])+l1*Ms*((-2)*torch.cos(x[3])+torch.cos(x[2]+x[3])))), 
            I1x+I2x+(1/4)*(l1**2*Ms+l2**2*(4*Ms+Mt))+l1*l2*Ms*torch.cos(x[3]), 
            I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3]))]), 

        torch.stack([I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3])+(-2)*l2*torch.cos(x[2]+x[3])+(-2)*l1*torch.cos(x[1]+x[2]+x[3])), 
            I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3])+(-2)*l2*torch.cos(x[2]+x[3])), 
            I1x+(1/4)*l1*Ms*(l1+2*l2*torch.cos(x[3])), 
            torch.tensor(I1x+(1/4)*l1**2*Ms, device = x.device)])
        ])
    return M.to(x.device)

def calculate_Nvect(x, device, Mp, Ms, Mt, l1, l2, g):
    N =  torch.stack([
         torch.stack([(1/2)*g*((-1)*l1*(2*Mp+3*Ms+4*Mt)*torch.sin(x[0])+(-1)*l2*(2*(Mp+Ms)+3*Mt)*torch.sin(x[0]+x[1])+l2*(2*Ms+Mt)*torch.sin(x[0]+x[1]+x[2])+l1*Ms*torch.sin(x[0]+x[1]+x[2]+x[3]))]), 
         torch.stack([(1/2)*g*((-1)*l2*(2*(Mp+Ms)+3*Mt)*torch.sin(x[0]+x[1])+l2*(2*Ms+Mt)*torch.sin(x[0]+x[1]+x[2])+l1*Ms*torch.sin(x[0]+x[1]+x[2]+x[3]))]), 
         torch.stack([(1/2)*g*(l2*(2*Ms+Mt)*torch.sin(x[0]+x[1]+x[2])+l1*Ms*torch.sin(x[0]+x[1]+x[2]+x[3]))]), 
         torch.stack([(1/2)*g*l1*Ms*torch.sin(x[0]+x[1]+x[2]+x[3])])
        ])
    return N
