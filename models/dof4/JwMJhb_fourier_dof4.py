...
from models.dof4.dynamics import I1z, I2z, Ipz, Mp, Ms, Mt, l1, l2, g, calculate_Mmtx, calculate_Nvect, calculate_J0, calculate_genM0, calculate_J1, calculate_genM1, calculate_J2, calculate_genM2, calculate_J3, calculate_genM3, calculate_J4, calculate_genM4
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        K_vec = 1.0 + alpha_x * bounded_output          # ∈ [0.7, 1.3] with alpha_x ≈ 1
    ...
    def calculate_M_hat(self, X):
        ...
        # Build batched J and genM by calling your existing single-sample functions
        J0  = torch.stack([calculate_J0(x_b[i])  for i in range(n)], dim=0)  # [n,6,4]
        gM0 = torch.stack([calculate_genM0(x_b[i]) for i in range(n)], dim=0) # [n,6,6]

        J1  = torch.stack([calculate_J1(x_b[i])  for i in range(n)], dim=0)  # [n,6,4]
        gM1 = torch.stack([calculate_genM1(x_b[i]) for i in range(n)], dim=0) # [n,6,6]

        J2  = torch.stack([calculate_J2(x_b[i])  for i in range(n)], dim=0)  # [n,6,4]
        gM2 = torch.stack([calculate_genM2(x_b[i]) for i in range(n)], dim=0) # [n,6,6]

        J3  = torch.stack([calculate_J3(x_b[i])  for i in range(n)], dim=0)  # [n,6,4]
        gM3 = torch.stack([calculate_genM3(x_b[i]) for i in range(n)], dim=0) # [n,6,6]

        J4  = torch.stack([calculate_J4(x_b[i])  for i in range(n)], dim=0)  # [n,6,4]
        gM4 = torch.stack([calculate_genM4(x_b[i]) for i in range(n)], dim=0) # [n,6,6]

        # Apply K weights per-batch (broadcast across 6x6)
        gM1 = gM1 * K_vec[:, 0].view(n, 1, 1)
        gM2 = gM2 * K_vec[:, 1].view(n, 1, 1)
        gM3 = gM3 * K_vec[:, 2].view(n, 1, 1)
        gM4 = gM4 * K_vec[:, 3].view(n, 1, 1)


        # construct shaped mass matrix using forward output K and mass matrix M
        ax0 = (J0.transpose(1, 2) @ gM0) @ J0
        ax1 = (J1.transpose(1, 2) @ gM1) @ J1
        ax2 = (J2.transpose(1, 2) @ gM2) @ J2
        ax3 = (J3.transpose(1, 2) @ gM3) @ J3
        ax4 = (J4.transpose(1, 2) @ gM4) @ J4

        M_hat = ax0+ax1+ax2+ax3+ax4  #[n,4,4]
        
        if unbatched:
            M_hat = M_hat.squeeze(0)  # [4, 4]
